...
// Defaults to 512 32-bit pixels max width
#define PNG_MAX_BUFFERED_PIXELS (480*4 + 1)
// PNG filter type
enum {
    PNG_FILTER_NONE=0,
    PNG_FILTER_SUB,
...
typedef struct png_file_tag
{
  ...
  int32_t fHandle; // class pointer to File/SdFat or whatever you want
} PNGFILE;
...
typedef struct png_image_tag
{
    ...
    uint8_t ucLineBuff[PNG_MAX_BUFFERED_PIXELS];
    uint16_t usLowColorBuffer[512*272];
